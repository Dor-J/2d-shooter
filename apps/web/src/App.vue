...
import { flagIndicators, limitLine, standingFor, teamScores, weaponPanel } from './hud/status.ts'
...
const weaponCorner = computed(() =>
  weaponPanel(weaponNames.value, selectedWeapon.value, null, me.value?.grenades ?? 0, (me.value as { cluster_grenades?: number } | undefined)?.cluster_grenades ?? 0),
)
...
function connect() {
  ...
  socket.onopen = () => send({ type: 'hello', version: 13, name: name.value.trim(), resume: sessionStorage.getItem('arena-token') })
...
<template>
  <div class="shell" :class="{ 'in-match': roomId !== null }">
    ...
    <main v-else-if="roomId === null" class="lobby">
      ...
      <div class="rooms"><div v-for="room in rooms" :key="room.id" class="room"><div><strong>{{ room.name }}</strong><span>{{ room.map }} · {{ room.weapon_mod || 'Default mod' }} · {{ modeLabel(room.mode) }} · {{ room.players }}/{{ room.capacity }} players</span><p v-if="badgesFor(room).length" class="room-badges"><span v-for="badge in badgesFor(room)" :key="badge">{{ badge }}</span></p></div><button :disabled="room.players >= room.capacity" @click="join(room.id)">Join</button></div></div>
