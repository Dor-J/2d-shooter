...
pub struct DefaultMap { pub name: &'static str, pub mode: MapMode }
...
pub fn original_default_map(entry: DefaultMap) -> MapAsset {
    let seed = entry.name.bytes().fold(0_u32, |value, byte| value.wrapping_mul(33).wrapping_add(u32::from(byte)));
    ...
    let polygons = vec![
        ...
        polygon([(180.0, platform_y), (520.0, platform_y - 30.0), (520.0, platform_y + 20.0)], 0),
        polygon([(680.0, platform_y + 20.0), (1020.0, platform_y - 10.0), (680.0, platform_y - 30.0)], if seed % 7 == 0 { 4 } else { 0 }),
    ];
    let mut spawnpoints = vec![spawn(260, 600, 0), spawn(940, 600, 0)];
    if matches!(entry.mode, MapMode::CaptureTheFlag | MapMode::Infiltration) {
        spawnpoints.extend([spawn(180, 600, 1), spawn(1020, 600, 2), spawn(100, 610, 5), spawn(1100, 610, 6)]);
    }
    MapAsset {
        hash: 0,
        version: 1,
        ...
        sectors_division: 50,
        sectors_num: 0,
        sectors: vec![Sector { polygons: vec![1, 2, 3, 4] }],
        props: Vec::new(),
        scenery: Vec::new(),
        colliders: Vec::new(),
        spawnpoints,
...
fn vertex(x: f32, y: f32) -> Vertex { Vertex { x, y, z: 0.0, rhw: 1.0, color: [180, 190, 190, 255], u: x / 1200.0, v: y / 700.0 } }
fn polygon(points: [(f32, f32); 3], kind: u8) -> Polygon { Polygon { vertices: points.map(|(x, y)| vertex(x, y)), normals: [[0.0, -1.0, 1.0]; 3], kind } }
fn spawn(x: i32, y: i32, team: i32) -> Spawnpoint { Spawnpoint { active: true, x, y, team } }
fn waypoint(id: i32, x: i32, y: i32, connection: i32) -> Waypoint { let mut connections = [0; 20]; connections[0] = connection; Waypoint { active: true, id, x, y, left: true, right: true, up: false, down: false, jetpack: true, path_num: 0, action: 0, connections_num: 1, connections } }
